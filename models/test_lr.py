...
class TestLR:
    ...
    def test_fit(self, y, x):
        m = lr.LR(0, 2)
        m.fit(y, x)
        assert abs(m.w[0] + 0.22692079) < 1e-6
        assert abs(m.w[1] + 15.21590892) < 1e-6
        assert abs(m.w[2] + (-30.08303294)) < 1e-6
        assert abs(m.w[3] + 14.45525843) < 1e-6
        assert abs(m.w[4] + 0.0845936) < 1e-6

...
def main(sample_path, size_train, size_test, predict_label_cnt):
    ...
    s = reader.read_sample(sample_path, size_train + size_test, 0)[1]
    tfidf_x_train = sparse_tf_idf.tf_idf(s.convert_to_csr(s.max_feature + 1))
    good_features = feature_selection.high_tf_idf_features(tfidf_x_train, 99)
    print len(good_features)
    fm = {f: i for i, f in enumerate(good_features)}
    reduction_x_train = feature_selection.construct_lower_rank_x(x_train, good_features, fm)

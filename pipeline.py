...
class PipeLine:
    ...
    def run(self, in_path):
        ...
        for param in product(self._threshold, self._predict_cnt):
            ...
            for i, part_y in enumerate(part_csr_y_generator(csr_mapped_y.transpose().tolil(), 100)):
                mnb = self._model()
                mnb.fit(csr_mapped_y, part_y, mapped_reduced_x)
                mapped_test_predicted_y = mnb.predict(i, 100, mapped_reduced_test_x, predict_cnt)
